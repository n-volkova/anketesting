<template>
    <section class="profile-step">
        <div class="intro">
            <p>Предлагаем тебе заполнить анкету и ответить на наши вопросы, чтобы мы начали знакомство с тобой. Сразу после анкеты тебя ждет два теста: на грамотность и на многозадачность. </p>
            ...
            <form @submit.prevent="submit">
                ...
                <div class="profile-form">
                    ...
                    <div class="input-wrapper js-input-wrapper">
                        <textarea 
                            class="js-input"
                            v-model.trim.lazy="profileData.startup"
                            type="text"
                            name="startup" 
                            :placeholder="startupPlaceholder"
                            rows="10"
                        />
                        <span class="error-message">{{ errors.first('startup') }}</span>
                    </div>

                    <div class="input-wrapper js-input-wrapper">
                        <textarea 
                            class="js-input"
                            v-model.trim.lazy="profileData.rocket"
                            v-validate="'required'"
                            type="text"
                            name="rocket" 
                            ref="rocket" 
                            :placeholder="rocketPlaceholder"
                            rows="10"
                            @focus="onInvalidFocus($event)"
                        />
                        <span class="error-message">{{ errors.first('rocket') }}</span>
                    </div>
...
export default {
    data() {
        return {
            ...
            origin_text: '',
            aboutPlaceholder: 'Расскажите, пожалуйста, что вы любите, чем занимаетесь в свободное время, есть ли у вас хобби, какие книги читаете, собака у вас или кошка. Нам интересно всё!',
            startupPlaceholder: 'Представьте, что у вас есть свой стартап, и в нем — служба поддержки клиентов. Опишите в нескольких предложениях, как бы вы организовали его работу.',
            rocketPlaceholder: 'Предположим, вы уже работаете в поддержке Рокетбанка. Друзья спрашивают: «Ну как?» и «Что ты там делаешь?». Что бы вы им ответили?',
